...
def detect_onsets(sig, fftwin = 512):
    ...
    # need to apply function to every row of the spectrogram
...
def generate_spectrogram(audio, window_size):
    ...
    for t in range(0, len(audio), window_size):
        if(len(audio) - t < window_size):   #added mod to ignore end
            break
        else:
...
# I believe this function will create an array of zeros
# and 1 spikes that represent onsets
def climb_hills(vector):
    moving_points = list( range(len(vector)) ) # had to turn this into a list
    temp = np.array(moving_points)
    stable_points = []

    # got to understand this loop...
    # for some reason, he is removing things from moving points and appending to stable?
